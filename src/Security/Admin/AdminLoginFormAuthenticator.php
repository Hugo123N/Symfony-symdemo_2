...
class AdminLoginFormAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function checkCredentials($credentials, UserInterface $user)
    {
        if ($this->passwordEncoder->isPasswordValid($user, $credentials['password'])) {
            if (!$this->isAdmin($user)) {
                throw new CustomUserMessageAuthenticationException('you are not a admin.');
            }
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        ...
        // redirect to some "app_homepage" route - dang nhap thanh cong thi chuyen den:
        return new RedirectResponse($this->urlGenerator->generate('listCategories'));
    }
